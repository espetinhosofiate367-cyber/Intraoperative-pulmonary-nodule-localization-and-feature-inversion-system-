...
paper_app_dir = os.path.join(os.path.dirname(__file__), 'paper_flexible_sensor_lung_nodule_localization', 'app')
if paper_app_dir not in sys.path:
    sys.path.append(paper_app_dir)
try:
...
class OptimizedDetectionGUI:
    def __init__(self, root):
        ...
        self.paper_stage2_ckpt_map = {
            "hierarchical": os.path.join(
                ...
                "paper_flexible_sensor_lung_nodule_localization",
                "experiments",
            ...
            "raw+delta": os.path.join(
                os.path.dirname(__file__),
                "paper_flexible_sensor_lung_nodule_localization",
                "experiments",
                "outputs_stage2_dualstream_mstcn_multitask_raw_delta",
                "paper_stage2_dualstream_mstcn_best.pth",
            ),
        }
    ...
    def _apply_ai_display_policy(self, result):
    ...
    def _run_ai_sequence_inference(self, seq_raw):
        ...
        try:
            if self.dl_backend == "paper_two_stage" and self.paper_ai_pipeline is not None:
                ...
                self._apply_ai_display_policy(result)
                return result
    ...
    def create_widgets(self):
        ...
        self.ai_model_combo = ttk.Combobox(
            ...
            values=["hierarchical", "raw", "raw+delta"],
            width=10,
            state="readonly",
        )
                ...
                if self.use_dl_detection and self.dl_var.get() and self.dl_model:
                    try:
                        ...
                        if len(self.frame_buffer) == self.seq_len:
                            ...
                            self._run_ai_sequence_inference(seq_raw)
                        else:
                            self._reset_ai_outputs(prob=0.0)
    ...
    def scan_full_file(self):
        ...
        try:
            ...
            for i in range(total_frames):
                ...
                result = self._run_ai_sequence_inference(seq_raw_reshaped)
    ...
    def update_ai_output_labels(self):
        ...
        if self.dl_backend == "paper_two_stage":
            ...
            self.ai_status_var.set(
                f"AI状态: 两阶段 | 反演={self.selected_stage2_variant} | 阈值={threshold:.3f}/{threshold_low:.3f} | 延迟={latency_ms:.1f} ms"
            )
        else:
        ...
        try:
            ...
            if self.use_dl_detection and self.dl_var.get() and self.dl_model:
                try:
                    ...
                    self._run_ai_sequence_inference(seq_raw_reshaped)
                except Exception as e:
