...
\begin{figure}[t]
    ...
    \caption{Size inversion comparison. The optimized raw-input size-only model exceeds the structured XGBoost baseline on top-1, top-2, and MAE, while the unified model remains a deployment-oriented reference rather than the scientific main model.}
...
For the current route-aware depth v2 model, latent probes reached a mean test $R^2$ of 0.2712, compared with 0.2356 for a size-only baseline, indicating that the network encoded depth-relevant structure beyond size identity alone. Integrated Gradients, computed on 40 correctly predicted test windows per depth class, showed class-related spatial attribution patterns rather than a single-point peak strategy. Hard-pair analysis still achieved a 0.7500 success rate, while phase occlusion showed that the largest mean probability drop remained concentrated around the peak neighborhood (0.2303), larger than loading-early or release masking. Together, these analyses supported the claim that raw-input learning had captured part---but not all---of the relevant physical structure.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{Fig10_hierarchical_explainability_summary.pdf}
    \caption{Interpretability summary for the current route-aware raw depth model. Latent probes, phase occlusion, Integrated Gradients, and hard-pair analysis jointly indicate that the network encodes physically meaningful depth-related structure, although its temporal strategy remains partially biased toward the peak neighborhood.}
