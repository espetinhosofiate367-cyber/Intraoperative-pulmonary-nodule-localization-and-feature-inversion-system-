...
def evaluate_model(model: nn.Module, loader: DataLoader, device: torch.device, criterion_cls: nn.Module, criterion_reg: nn.Module) -> Dict[str, object]:
    model.eval()
    total_loss = 0.0
    total_n = 0
    ...
    with torch.no_grad():
        for raw_x, norm_x, size_idx, size_cm in loader:
            ...
            size_logits, size_reg, _size_probs = model(raw_x, norm_x)
            loss_cls = criterion_cls(size_logits, size_idx)
            loss_reg = criterion_reg(size_reg.squeeze(1), size_cm)
            ...
            reg_all.append(size_reg.squeeze(1).cpu().numpy().astype(np.float64))
...
@dataclass
class Config:
    seed: int = int(os.environ.get("PAPERSIZE_SEED", "2026"))
    seq_len: int = int(os.environ.get("PAPERSIZE_SEQ_LEN", str(INPUT_SEQ_LEN)))
    stride: int = int(os.environ.get("PAPERSIZE_STRIDE", str(WINDOW_STRIDE)))
    dedup_gap: int = int(os.environ.get("PAPERSIZE_DEDUP_GAP", "6"))
    epochs: int = int(os.environ.get("PAPERSIZE_EPOCHS", "90"))
    batch_size: int = int(os.environ.get("PAPERSIZE_BATCH_SIZE", "48"))
    eval_batch_size: int = int(os.environ.get("PAPERSIZE_EVAL_BATCH_SIZE", "128"))
    lr: float = float(os.environ.get("PAPERSIZE_LR", "2e-4"))
    weight_decay: float = float(os.environ.get("PAPERSIZE_WEIGHT_DECAY", "1e-3"))
    dropout: float = float(os.environ.get("PAPERSIZE_DROPOUT", "0.25"))
    frame_feature_dim: int = int(os.environ.get("PAPERSIZE_FRAME_DIM", "24"))
    temporal_channels: int = int(os.environ.get("PAPERSIZE_TEMPORAL_CHANNELS", "48"))
    temporal_blocks: int = int(os.environ.get("PAPERSIZE_TEMPORAL_BLOCKS", "3"))
    patience: int = int(os.environ.get("PAPERSIZE_PATIENCE", "18"))
    grad_clip: float = float(os.environ.get("PAPERSIZE_GRAD_CLIP", "1.0"))
    aug_noise_std: float = float(os.environ.get("PAPERSIZE_AUG_NOISE_STD", "0.01"))
    aug_scale_jitter: float = float(os.environ.get("PAPERSIZE_AUG_SCALE_JITTER", "0.06"))
    aug_frame_dropout: float = float(os.environ.get("PAPERSIZE_AUG_FRAME_DROPOUT", "0.02"))
    label_smoothing: float = float(os.environ.get("PAPERSIZE_LABEL_SMOOTH", "0.03"))
    num_workers: int = int(os.environ.get("PAPERSIZE_NUM_WORKERS", "0"))
    ...
    def __post_init__(self):
        ...
        self.data_root = os.environ.get("PAPERSIZE_DATA_ROOT", os.path.join(REPO_ROOT, "整理好的数据集", "建表数据"))
        self.file1_labels = os.environ.get("PAPERSIZE_FILE1_LABELS", os.path.join(REPO_ROOT, "manual_keyframe_labels.json"))
        self.file2_labels = os.environ.get("PAPERSIZE_FILE2_LABELS", os.path.join(self.data_root, "manual_keyframe_labels_file2.json"))
        self.file3_labels = os.environ.get("PAPERSIZE_FILE3_LABELS", os.path.join(self.data_root, "manual_keyframe_labels_file3.json"))
        self.output_dir = os.environ.get(
            "PAPERSIZE_OUTPUT_DIR",
            os.path.join(PROJECT_DIR, "experiments", "outputs_stage2_raw_positive_size"),
        )
...
def main():
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        val_metrics = evaluate_model(model, val_loader, device, criterion_cls, criterion_reg)
        test_metrics = evaluate_model(model, test_loader, device, criterion_cls, criterion_reg)
    ...
    torch.save(
        {
            ...
            "router_model_name": "RawPositiveSizeModel",
        },
        os.path.join(cfg.output_dir, "paper_stage2_raw_positive_size_best.pth"),
    )

    plot_curves(history, os.path.join(cfg.output_dir, "paper_stage2_raw_positive_size_curves.png"))

    summary = {
        "protocol_v1": protocol_summary(),
        "model_name": "RawPositiveSizeModel",
        "validation_protocol": "file1_train_file2_val_file3_test_positive_windows",
        "best_epoch": int(best["epoch"]),
        "val_metrics": best["val_metrics"],
        "test_metrics": best["test_metrics"],
        "curve_path": os.path.join(cfg.output_dir, "paper_stage2_raw_positive_size_curves.png"),
    }
    with open(os.path.join(cfg.output_dir, "paper_stage2_raw_positive_size_summary.json"), "w", encoding="utf-8") as f:
