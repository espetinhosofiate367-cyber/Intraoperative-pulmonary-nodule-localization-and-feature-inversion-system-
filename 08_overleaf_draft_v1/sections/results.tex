...
\subsection{Raw Tensors Already Carried Strong Size Information}
For size, the raw size-only router achieved top-1 0.6600 and top-2 0.8405, demonstrating that the original tensors already carried strong size information. However, the best standalone size regression MAE remained with XGBoost (0.1472 cm), indicating that strong raw-input classification evidence did not necessarily imply the best isolated regression performance.
...
When depth was modeled with an ordinary shared head, performance collapsed to near-majority level (balanced accuracy around 0.33). In contrast, a size-routed raw-input depth model improved the ground-truth-route balanced accuracy to 0.5238, slightly above the XGBoost depth baseline of 0.5138. This result showed that the failure of early raw-input depth models was primarily structural rather than informational.
...
Although the raw size-routed depth model performed well under oracle routing, predicted-size routing reduced depth balanced accuracy to 0.4822. This analysis demonstrated that the main bottleneck in deployment was not the depth trunk itself, but the propagation of routing errors from size prediction.

\subsection{The Unified Hierarchical Inverter Improved Predicted-Route System Performance}
The deployment-oriented unified hierarchical inverter directly optimized route-aware performance and improved predicted-route hard balanced accuracy to 0.5337. This exceeded both the previous predicted-route chain (0.4822) and the XGBoost depth baseline (0.5138), showing that structured deployment enhancement could improve the robustness of the full system.
