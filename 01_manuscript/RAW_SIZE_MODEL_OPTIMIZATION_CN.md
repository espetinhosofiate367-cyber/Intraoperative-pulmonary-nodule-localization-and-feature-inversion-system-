...
## 当前结论先说清楚
**这条线已经优化成功，而且结果已经真正落地。**

这次优化后的结论是：

1. 纯 `raw-input size-only` 模型已经不再只是“接近 XGBoost”
2. 它现在在 `Top-1 / Top-2 / MAE` 上都已经超过 XGBoost
3. 这证明问题的关键确实主要在 **size head 与训练目标设计**，而不是 backbone 完全学不到大小信息
...
| Raw size-only router v1 | 0.6600 | **0.8405** | 0.2907 |
| Raw size-only router v2 | **0.7177** | **0.8195** | **0.1242** |

这说明：
- 原始时空张量里，**大小分类和连续大小回归都可以学出来**
- 经过输出头和损失函数重构后，pure raw scientific line 已经能跨过结构化基线

因此，这份文档现在更适合作为一份“优化路径总结”：

> **保留纯原始输入前提下，把 size 输出头和训练目标改成更贴近有序尺寸估计的形式，能够把 pure raw size 路线真正推过 XGBoost。**
