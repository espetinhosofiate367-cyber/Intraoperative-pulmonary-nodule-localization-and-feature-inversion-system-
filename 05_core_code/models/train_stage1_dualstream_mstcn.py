...
from train_stage1_detection import (
    build_center_label_samples_for_file,
...
class Config:
    ...
    epochs: int = int(os.environ.get("PAPERDSM_EPOCHS", "160"))
    batch_size: int = int(os.environ.get("PAPERDSM_BATCH_SIZE", "64"))
    lr: float = float(os.environ.get("PAPERDSM_LR", "8e-5"))
    ...
    patience: int = int(os.environ.get("PAPERDSM_PATIENCE", "28"))
    ...
    use_delta_branch: bool = env_bool("PAPERDSM_USE_DELTA_BRANCH", True)
    ...
    def __post_init__(self):
        ...
        self.output_dir = os.environ.get(
            "PAPERDSM_OUTPUT_DIR",
            os.path.join(PROJECT_DIR, "experiments", f"outputs_stage1_dualstream_mstcn_detection_{self.variant_name}"),
        )
...
def main():
    ...
    rec1, samples1 = build_center_label_samples_for_file(
        file1_all, "1.CSV", cfg.data_root, cfg.seq_len, cfg.stride, cfg.dedup_gap
    )
    rec2, samples2 = build_center_label_samples_for_file(
        file2_all, "2.CSV", cfg.data_root, cfg.seq_len, cfg.stride, cfg.dedup_gap
    )
    rec3, samples3 = build_center_label_samples_for_file(
        file3_all, "3.CSV", cfg.data_root, cfg.seq_len, cfg.stride, cfg.dedup_gap
    )
    ...
    ds_train = SingleSequenceDataset(
        train_records,
        train_samples,
        is_train=True,
    ...
    ds_train_eval = SingleSequenceDataset(train_records, train_samples_all, is_train=False)
    ds_val = SingleSequenceDataset(val_records, val_samples, is_train=False)
    ds_test = SingleSequenceDataset(test_records, test_samples, is_train=False)
    ...
    raw_pos_weight = float(np.sum(y_train == 0) / max(np.sum(y_train == 1), 1))
    pos_weight = float(max(1.0, raw_pos_weight * cfg.train_pos_weight_scale))
    ...
    for epoch in range(1, cfg.epochs + 1):
        tr = run_epoch(model, loader_train, device, criterion_train, optimizer=optimizer, grad_clip=cfg.grad_clip)
        ...
        auc_val = build_roc(val_eval["y_true"], val_eval["y_score"])
        ap_val = build_pr(val_eval["y_true"], val_eval["y_score"])
        rec = {
            "epoch": int(epoch),
    ...
    torch.save(build_checkpoint(model, cfg, history[-1]["epoch"] if history else 0, best_record=best_rec), last_path)
    ...
    state_dict = best_state["model_state_dict"] if isinstance(best_state, dict) and "model_state_dict" in best_state else best_state
    ...
    summary = {
        ...
        "task_definition": {
            ...
            "input_unit": "length-T tactile stress sequence window",
            "label_rule": "center_frame_inside_positive_segment",
            "split_protocol": "1+2 development, 3 final test, group-level split only",
            "architecture": "dual_stream_spatial_cnn_mstcn_detection",
            "streams": ["raw", "delta"] if cfg.use_delta_branch else ["raw"],
        },
        "config": {
            "seed": cfg.seed,
            "seq_len": cfg.seq_len,
            ...
            "loss_function_train": "BCEWithLogitsLoss(pos_weight)",
            "loss_function_eval": "BCEWithLogitsLoss(plain)",
            "label_mode": "center_frame_positive",
            "use_delta_branch": bool(cfg.use_delta_branch),
            "protocol_v1": protocol_summary(),
            "train_files": ["1.CSV", "2.CSV"],
            "validation_files": ["1.CSV", "2.CSV"],
            "test_file": "3.CSV",
        ...
        "split": {
            ...
            "train_domain_files": 2,
            "validation_domain_files": 2,
            "test_domain_files": 1,
            ...
            "train_group_count_file1": int(len(rec1)),
            "train_group_count_file2": int(len(rec2)),
            "test_group_count_file3": int(len(rec3)),
        },
